...
if __name__ == "__main__" :
    ...
    incoming_start_message_handler = MessageHandler(
        incoming_start_message_f,
        filters=Filters.command([Command.START]) & Filters.chat
    )
    ...
    incoming_compress_message_handler = MessageHandler(
        incoming_compress_message_f,
        filters=Filters.command([Command.COMPRESS]) & Filters.chat
    )
    ...
    incoming_cancel_message_handler = MessageHandler(
        incoming_cancel_message_f,
        filters=Filters.command([Command.CANCEL]) & Filters.chat
    )
    ...
    exec_message_handler = MessageHandler(
        exec_message_f,
        filters=Filters.command([Command.EXEC]) & Filters.chat
    )
    ...
    help_text_handler = MessageHandler(
        help_message_f,
        filters=Filters.command([Command.HELP]) & Filters.chat
    )
    ...
    upload_log_f_handler = MessageHandler(
        upload_log_file,
        filters=Filters.command([Command.UPLOAD_LOG_FILE]) & Filters.chat
    )
